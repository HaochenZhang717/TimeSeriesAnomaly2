...
for model_size in "${model_sizes[@]}"; do
  ...
  python classification.py \
      --mode "linear_probing" \
      --output_path "/root/tianyi/moment_eval/qtdb/timevae/${model_size}" \
      --train_data_path "/root/tianyi/formal_experiment/qtdb_one_channel/TimeVAE/no_code_impute_from_scratch_ckpt_lr1e-4/no_code_impute_samples.pth"\
