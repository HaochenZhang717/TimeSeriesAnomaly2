...
def evaluate_finetune_anomaly_quality():
    ...
    if args.need_to_generate:
        ...
        torch.save(all_samples,f"{args.generated_path}/generated_anomaly.pt")
    else:
...
def evaluate_finetune_normal_quality():
    args = get_evaluate_args()
    ...
    if args.need_to_generate:
        ...
        normal_train_iterator = iter(normal_train_loader)
        for _ in tqdm(range(num_cycle), desc="Generating samples"):
            anomaly_label = next(normal_train_iterator)['anomaly_label'].to(device)
            assert anomaly_label.sum().item() == 0
            samples = model.generate_mts(
                batch_size=args.batch_size,
                anomaly_label=anomaly_label,
            ).cpu()
...
if __name__ == "__main__":
    evaluate_finetune_normal_quality()
