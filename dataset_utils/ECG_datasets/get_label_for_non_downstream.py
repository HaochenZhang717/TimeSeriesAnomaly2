...
def mitdb():
    path = "./indices/slide_windows_106npz/train/V_train.jsonl"
    records = []
...
def qtdb():
    ...
    torch.save(to_save, "test_data/qtdb233_train_data.pt")


def svdb():
    path = "./indices_svdb/slide_windows_859npz/V_train.jsonl"
    records = []
    with open(path, "r") as f:
        for line in f:
            records.append(json.loads(line))
    ...
    all_samples = []
    all_labels = []
    raw_data = np.load("./raw_data_svdb/859.npz")
    raw_signal = raw_data["signal"]
    anomaly_label = raw_data["anomaly_label"]
    ...
    torch.save(to_save, "test_data/svdb859_train_data.pt")
