...
class NormalTimeVAEDecoder(nn.Module):
    def __init__(self,
        ...
        super(NormalTimeVAEDecoder, self).__init__()
        self.seq_len = seq_len
        self.feat_dim = feat_dim
        self.hidden_layer_sizes = hidden_layer_sizes
        self.latent_dim = latent_dim
...
class AnomalyTimeVAEDecoder(nn.Module):
    def __init__(self, seq_len, feat_dim, hidden_layer_sizes, latent_dim,
        ...
        # 基础结构：level + trend + season + residual
        self.level_model = LevelModel(self.latent_dim, self.feat_dim, self.seq_len)
        self.trend_layer = None
        self.season_layer = None
        self.residual_conn = None
        ...
        if use_residual_conn:
            self.residual_conn = ResidualConnection(seq_len, feat_dim, hidden_layer_sizes, latent_dim,
        ...
        self.anomaly_fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(latent_dim * 2, feat_dim)
        )

    def forward(self, z, cond_mask):
        """
        z: latent vector (B, latent_dim)
        cond_mask: anomaly mask (B, seq_len, 1)  -> 1 for anomaly, 0 for normal
        """
        # Step 1: 正常信号重建
        outputs = self.level_model(z)
        if self.trend_layer is not None:
        ...
        # Step 2: 在最后一层注入条件扰动
        # 生成 anomaly perturbation，仅在 cond_mask=1 处起作用
        B, T, _ = cond_mask.shape
        perturb = self.anomaly_fc(z).unsqueeze(1).expand(-1, T, -1)  # (B, T, feat_dim)
        outputs = outputs + perturb * cond_mask

        return outputs


class TimeVAECGATS(BaseVariationalAutoencoder):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        super(TimeVAECGATS, self).__init__(**kwargs)
        ...
        self.normal_decoder = self._get_normal_decoder()
        self.anomaly_decoder = self._get_anomaly_decoder()
    ...
    def _get_normal_decoder(self):
        return NormalTimeVAEDecoder(
    ...
    def _get_anomaly_decoder(self):
        return AnomalyTimeVAEDecoder(
            self.seq_len, self.feat_dim, self.hidden_layer_sizes,
            self.latent_dim, self.trend_poly, self.custom_seas,
            self.use_residual_conn, self.encoder.encoder_last_dense_dim)
