...
class Diffusion_TS(nn.Module):
    ...
    def output(self, x, t, attn_mask):
        trend, season = self.model(x, t, padding_masks=attn_mask)
    ...
    def _no_code_imputation_loss(self, signals, attn_mask, noise_mask):
        """
        signals    : (B, T, C)   clean time series (x0)
        attn_mask  : (B, T)      padding / valid mask
        noise_mask : (B, T)      1 = missing/anomaly, 0 = observed
        """
        ...
        x = self.q_sample(x_start=signals, t=t, noise=noise)  # noise sample

        x = x * noise_mask.unsqueeze(-1) + (1-noise_mask).unsqueeze(-1) * signals # only apply mask to missing area

        model_out = self.output(x, t, attn_mask)

        model_out = model_out * noise_mask.unsqueeze(-1)
        target = signals * noise_mask.unsqueeze(-1)
        ...
        loss_per_sample = loss_per_sample * extract(self.loss_weight, t, loss_per_sample.shape)
    ...
    @torch.no_grad()
    def no_code_impute(
            self, signals, attn_mask,
            noise_mask, clip_denoised=True
    ):

        batch, device, total_timesteps, sampling_timesteps, eta = \
            signals.shape[0], self.betas.device, self.num_timesteps, self.sampling_timesteps, self.eta
        ...
        times = list(reversed(times.int().tolist()))
        time_pairs = list(zip(times[:-1], times[1:]))  # [(T-1, T-2), (T-2, T-3), ..., (1, 0), (0, -1)]
        img = torch.randn_like(signals)
        img = img * noise_mask.unsqueeze(-1) + (1 - noise_mask).unsqueeze(-1) * signals
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            if time_next < 0:
                img = x_start * noise_mask.unsqueeze(-1) + signals * (1 - noise_mask).unsqueeze(-1)
                continue
