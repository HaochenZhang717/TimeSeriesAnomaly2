...
from src.TimeVAE_CGATS.vae.vae_dense_model import VariationalAutoencoderDense as VAE_Dense
from src.TimeVAE_CGATS.vae.vae_conv_model import VariationalAutoencoderConv as VAE_Conv
from src.TimeVAE_CGATS.vae.timevae import TimeVAE
...
def train_vae(vae, train_data, valid_data, max_steps, lr, verbose=0):
    """
    Train a VAE model.
    ...
    """
    vae.fit_on_data(train_data, valid_data, max_steps, lr, verbose)


def pretrain_vae(vae, train_data, valid_data, max_steps, lr, save_dir, verbose=0):
    vae.pretrain(train_data, valid_data, max_steps, lr, verbose, save_dir)

def finetune_vae(vae, train_data, valid_data, max_steps, lr, save_dir, verbose=0):
    vae.finetune(train_data, valid_data, max_steps, lr, verbose, save_dir)
...
def get_posterior_samples_normal(vae, data):
    """
    ...
    """
    return vae.normal_predict(data)


def get_posterior_samples_anomaly(vae, data):
    """
    Get posterior samples from the VAE model.
    ...
    Returns:
        np.ndarray: The posterior samples.
    """
    return vae.anomaly_predict(data)


def get_prior_anomaly_samples(vae, num_samples: int):
    """
    ...
    """
    return vae.get_prior_anomaly_samples(num_samples=num_samples)
