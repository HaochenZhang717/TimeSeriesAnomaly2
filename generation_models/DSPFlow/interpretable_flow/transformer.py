...
class TrendBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, in_feat, out_feat, act):
        ...
        #     nn.Conv1d(in_channels=in_dim, out_channels=self.trend_poly, kernel_size=3, padding=1),
        #     act,
        #     Transpose(shape=(1, 2)),
        ...
        self.trend = nn.Sequential(
            ...
            act,
            nn.Linear(in_feat//2, out_feat*self.trend_poly),
        )
    ...
    def forward(self, input):
        ...
        input = input.mean(dim=1) # (batch_size, hidden_dim)

        x = self.trend(input).reshape(batch_size, self.trend_poly, -1)

        # x = self.trend(input).transpose(1, 2) #(batch_size, 3, ts_dim, )

        lin_space = torch.arange(1, seq_len + 1, 1) / (seq_len + 1)
        poly_space = torch.stack([lin_space ** float(p + 1) for p in range(self.trend_poly)], dim=0)


        trend_vals = torch.matmul(x.transpose(1, 2), poly_space.to(x.device))
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.ln1_1 = AdaLayerNorm(n_embd, proto_dim)
        ...
        self.proj = nn.Linear(n_embd, n_embd * 2)
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(n_layer_enc, n_embd, n_heads, attn_pdrop, resid_pdrop, mlp_hidden_times, block_activate,
                               max_len=self.max_len, proto_dim=proto_dim)
